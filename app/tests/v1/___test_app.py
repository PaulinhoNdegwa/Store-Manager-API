...
def authenticate(client):
    """Function successful signs up and logs in the client in order to access
     other operations It returns a login authentication token"""
...
def test_get_all_products(client):
    ...
    access_token = authenticate(client)
    response = client.get(
        "api/v1/products", headers=dict(Authorization="Bearer "+access_token))
...
def test_get_all_sales(client):
    ...
    access_token = authenticate(client)
    response = client.get(
        "api/v1/sales", headers=dict(Authorization="Bearer "+access_token))
...
def test_single_sale_order(client):
    ...
    access_token = authenticate(client)
    response = client.get(
        "api/v1/sales/1", headers=dict(Authorization="Bearer "+access_token))
    data = json.loads(response.data)["status"]
    assert data == 200
...
def test_order_excess(client):
    """Tests if the api creates a sale order of more items than the available
    products"""
    access_token = authenticate(client)
    sale = {
        "product_name": "Macbook",
...
def test_get_nonint_sale_productid(client):
    ...
    access_token = authenticate(client)
    response = client.get(
        "/api/v1/sales/j", headers=dict(Authorization="Bearer "+access_token))
    assert json.loads(response.data)["status"] == 404
