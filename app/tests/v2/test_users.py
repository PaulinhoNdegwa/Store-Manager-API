...
class UsersTestCase(BaseTest):
    ...
    def test_get_all_users(self):
        ...
        response = self.client.get("/api/v2/users", headers={'Content-Type': 'application/json',
    ...
    def test_get_one_user(self):
        ...
        access_token = self.authenticateAdmin()
        response = self.client.get("/api/v2/users/1", headers={'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + access_token})
    ...
    def test_get_inexistent_user(self):
        ...
        access_token = self.authenticateAdmin()
        response = self.client.get("/api/v2/users/10", headers={'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + access_token})
    ...
    def test_get_nonint_user_id(self):
        ...
        access_token = self.authenticateAdmin()
        response = self.client.get("/api/v2/users/qw", headers={'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + access_token})
        self.assertEqual(json.loads(response.data)["status"], 404)
